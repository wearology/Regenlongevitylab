...
const fdaReferenceSource = compileComponent('../components/fda-medicine-reference.tsx')
...
// Expand only the medicine-reference boundary for integration checks. Existing
// package/price tests stay focused; these checks also run the actual nested FDA
// component so a non-interactive mock cannot conceal an extra link or claim.
function resolveMedicineReferences(node, Reference) {
  if (node == null || typeof node !== 'object') return node
  if (Array.isArray(node)) return node.map((child) => resolveMedicineReferences(child, Reference))
  if (node.type === 'FdaMedicineReference') return Reference(node.props)
  if (!node.props || !Object.hasOwn(node.props, 'children')) return node
  return { ...node, props: { ...node.props, children: resolveMedicineReferences(node.props.children, Reference) } }
}

function renderedMedicineReferences(tree) {
  return descendants(tree).filter((node) => Object.hasOwn(node.props ?? {}, 'data-fda-medicine-reference'))
}
...
function createHarness(regionId) {
  ...
  const dependencies = {
    ...
    'lucide-react': {
      ...
      ImageOff: 'ImageOff',
      Info: 'ReferenceInfo', ExternalLink: 'ReferenceExternalLink',
    },
    '@/components/consultation-button': { ConsultationButton: 'ConsultationButton' },
    '@/components/fda-medicine-reference': { FdaMedicineReference: 'FdaMedicineReference' },
  ...
  const { FdaMedicineReference } = loadComponent(fdaReferenceSource, dependencies)
  ...
  return {
    ...
    withMedicineReferences(tree) { return resolveMedicineReferences(tree, FdaMedicineReference) },
    openInquiry(tree) {
...
test('actual product cards integrate exactly one compact Tesamorelin medicine reference in every region without nested links or changes to catalog prices and photos', () => {
  for (const region of REGIONS) {
    const harness = createHarness(region.id)
    const copy = productCopy.catalogCopy[region.language]
    for (const product of productData.products) {
      const rawCard = harness.renderCard(product)
      const boundaries = descendants(rawCard).filter((node) => node.type === 'FdaMedicineReference')
      assert.ok(boundaries.length <= 1, `${region.id}/${product.slug}: do not duplicate the reference`)
      for (const boundary of boundaries) {
        assert.equal(boundary.props.product, product)
        assert.notEqual(boundary.props.detail, true, 'a linked catalog card must use the non-interactive compact reference')
      }
      const card = harness.withMedicineReferences(rawCard)
      const references = renderedMedicineReferences(card)
      const isReviewed = product.slug === 'tesamorelin' && product.name === 'Tesamorelin 10mg'
      assert.equal(references.length, isReviewed ? 1 : 0, `${region.id}/${product.slug}`)
      assert.equal(card.type, 'RegionalLink')
      assert.equal(card.props.href, `/${region.id}/product/${product.slug}`)
      const links = descendants(card).filter((node) => ['a', 'RegionalLink'].includes(node.type))
      assert.deepEqual(links, [card], 'the card itself remains the only anchor; no FDA source link is nested inside it')
      assertCardImage(card, product, region.language)

      const variant = region.id === 'id' ? 'basic' : 'cartridge'
      const expectedPrice = region.id === 'id'
        ? `mulai dari ${indonesianPriceLabels[product.slug].basic}`
        : region.id === 'eu' ? 'EUR 143'
          : region.language === 'ms' ? 'Tanya harga MYR' : `Request ${region.currency} pricing`
      assert.deepEqual(priceAndUnit(card), { price: expectedPrice, unit: `/ ${copy.variants[variant].label.toLowerCase()}` })
      const localized = productCopy.getProductCopy(product, region.language)
      for (const unchanged of [product.name, localized.category, localized.tagline, copy.viewProduct]) {
        assert.ok(normalizedContent(card).includes(unchanged))
      }
      if (isReviewed) {
        const reference = references[0]
        assert.equal(reference.props['data-fda-medicine-reference'], 'compact')
        assert.equal(reference.props['data-reference-scope'], 'reference-medicine-only')
        assert.ok(content(reference).includes('FDA-approved medicine: EGRIFTA WR'))
        assert.ok(content(reference).includes(fdaReferences.FDA_REFERENCE_COPY[region.language].disclaimer))
        const text = normalizedContent(card)
        assert.ok(text.indexOf(localized.tagline) < text.indexOf('FDA-approved medicine:'))
        assert.ok(text.indexOf('FDA-approved medicine:') < text.indexOf(expectedPrice))
      } else {
        assert.doesNotMatch(content(card), /FDA-approved medicine:|EGRIFTA WR/)
      }
    }
    assert.deepEqual(harness.consultationRequests, [], 'showing a medicine reference does not initiate a consultation')
  }
})

test('actual product details retain the medicine-only reference through regional package changes without altering prices, galleries, research disclaimers or inquiries', () => {
  for (const region of REGIONS) {
    const copy = productCopy.catalogCopy[region.language]
    const variants = region.id === 'id' ? indonesianVariants : ['cartridge', 'pen']
    for (const product of productData.products) {
      const harness = createHarness(region.id)
      const quote = region.language === 'ms' ? 'Tanya harga MYR' : `Request ${region.currency} pricing`
      const prices = region.id === 'id' ? indonesianPriceLabels[product.slug]
        : region.id === 'eu' ? { cartridge: 'EUR 143', pen: 'EUR 200' }
          : { cartridge: quote, pen: quote }
      const isReviewed = product.slug === 'tesamorelin' && product.name === 'Tesamorelin 10mg'
      let rawDetail = harness.renderDetail(product)
      for (const variant of variants) {
        const label = copy.variants[variant].label
        formatButton(rawDetail, label).props.onClick()
        rawDetail = harness.renderDetail(product)
        const boundaries = descendants(rawDetail).filter((node) => node.type === 'FdaMedicineReference')
        assert.ok(boundaries.length <= 1)
        for (const boundary of boundaries) {
          assert.equal(boundary.props.product, product, 'the selected package must not replace the reviewed catalog identity')
          assert.equal(boundary.props.detail, true)
        }
        const detail = harness.withMedicineReferences(rawDetail)
        const references = renderedMedicineReferences(detail)
        assert.equal(references.length, isReviewed ? 1 : 0, `${region.id}/${product.slug}/${variant}`)
        assert.deepEqual(priceAndUnit(detail), { price: prices[variant], unit: `/ ${label.toLowerCase()}` })
        assertFormatButtons(detail, region.language, prices, variant)
        assert.equal(primaryImage(detail).props.src, variant === 'basic' ? product.image
          : variant === 'cartridge' ? '/products/cartridge-package.jpeg' : '/products/pen-package.jpeg')
        assert.equal(galleryButtons(detail).length, 3)
        assert.ok(content(detail).includes(copy.researchOnly), 'the FDA reference never replaces the research-only disclaimer')
        const expectedInquiry = region.id === 'id' ? `${product.name} — ${label}` : product.name
        assert.equal(inquiryButton(detail).props.productName, expectedInquiry)
        assert.equal(harness.openInquiry(detail), expectedInquiry)
        assert.doesNotMatch(harness.consultationRequests.at(-1), /FDA|EGRIFTA/,
          'consultation requests remain for the Regen research package, not the approved medicine')
        if (isReviewed) {
          assert.equal(references[0].props['data-fda-medicine-reference'], 'detail')
          assert.equal(references[0].props['data-reference-scope'], 'reference-medicine-only')
          assert.ok(content(references[0]).includes(fdaReferences.FDA_REFERENCE_COPY[region.language].disclaimer))
          assert.ok(content(references[0]).includes(fdaReferences.FDA_REFERENCE_COPY[region.language].distinction))
        } else {
          assert.doesNotMatch(content(detail), /FDA-approved medicine:|EGRIFTA WR/)
        }
      }
    }
  }

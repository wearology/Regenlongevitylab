...
function assertNoFdaUi(tree, context) {
  assert.doesNotMatch(content(tree), /\bFDA\b|EGRIFTA/i, `${context}: no FDA banner, badge or medicine-reference copy`)
  for (const node of descendants(tree)) {
    if (typeof node.type === 'string') assert.doesNotMatch(node.type, /FDA|EGRIFTA/i, context)
    for (const [name, value] of Object.entries(node.props ?? {})) {
      assert.doesNotMatch(name, /^data-fda|^data-reference-scope$/i, `${context}: no medicine-reference markers`)
      if (typeof value === 'string') {
        assert.doesNotMatch(value, /\bFDA\b|EGRIFTA|fda[-_](?:medicine|reference|badge|banner)|reference-medicine-only/i,
          `${context}: no FDA source links or hidden reference metadata in ${name}`)
      }
    }
  }
}
...
test('all eight actual product cards omit FDA badges, banners and links in all seven regions while retaining catalog prices, photos and localized copy', () => {
  assert.equal(productData.products.length, 8)
  ...
  for (const region of REGIONS) {
    ...
    for (const product of productData.products) {
      const card = harness.renderCard(product)
      assertNoFdaUi(card, `${region.id}/${product.slug}`)
      ...
      assert.deepEqual(links, [card], 'the product-detail link remains the only anchor in each card')
      assertCardImage(card, product, region.language)
    ...
    assert.deepEqual(harness.consultationRequests, [], 'browsing the catalog does not initiate a consultation')
  }
})

test('all actual product details stay free of FDA references through every regional package selection while preserving prices, galleries, research notices and inquiries', () => {
  assert.equal(productData.products.length, 8)
  assert.equal(REGIONS.length, 7)
  for (const region of REGIONS) {
    const copy = productCopy.catalogCopy[region.language]
    ...
    for (const product of productData.products) {
      ...
      const prices = region.id === 'id' ? indonesianPriceLabels[product.slug]
        : region.id === 'eu' ? { cartridge: 'EUR 143', pen: 'EUR 200' }
          : { cartridge: quote, pen: quote }
      const localized = productCopy.getProductCopy(product, region.language)
      let detail = harness.renderDetail(product)
      assertNoFdaUi(detail, `${region.id}/${product.slug}/default`)
      for (const variant of variants) {
        const label = copy.variants[variant].label
        formatButton(detail, label).props.onClick()
        detail = harness.renderDetail(product)
        assertNoFdaUi(detail, `${region.id}/${product.slug}/${variant}`)
        const links = descendants(detail).filter((node) => ['a', 'RegionalLink'].includes(node.type))
        assert.equal(links.length, 1, 'the regional back-to-catalog link is the only link in the product detail')
        assert.equal(links[0].type, 'RegionalLink')
        assert.equal(links[0].props.href, `/${region.id}#katalog`)
        assert.equal(content(links[0]), copy.backToCatalog)
        ...
        const gallery = galleryButtons(detail)
        assert.deepEqual(gallery.map((button) => descendants(button).find((node) => node.type === 'ProductImage').props.src),
          ['/products/cartridge-package.jpeg', '/products/pen-package.jpeg', product.image])
        const photoNotice = variant === 'basic' ? copy.basicPhotoNotice ?? copy.photoNotice : copy.photoNotice
        for (const unchanged of [product.name, localized.category, localized.description, copy.selectFormat,
          copy.priceNotice, copy.orderConsultation, copy.researchOnly, photoNotice, ...copy.usps]) {
          assert.ok(content(detail).includes(unchanged), `${region.id}/${product.slug}/${variant}: preserve ${unchanged}`)
        }
        ...
        assert.doesNotMatch(harness.consultationRequests.at(-1), /FDA|EGRIFTA/,
          'consultation requests remain for the selected Regen research package')
      }
    }
  }
})
